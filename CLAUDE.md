...
MCP server for Skylight Calendar — 29 tools across calendar events (read+write), shared lists (read+write), chores and rewards (read+write), and frame/device/account info (read). **Meals are not supported** — Skylight does not expose a meals API on this account type (all meals endpoints return 404). Do not create a `meals.ts` module.
...
29 tools total. 9 read-only frame/device/account tools, 8 event tools, 8 list tools (2R+6W), 4 chore/reward tools (2R+2W).

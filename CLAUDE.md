...
- **Tools register with the OLD `server.tool(name, description, schema, handler)` signature** — all 113 of them, zero `server.registerTool(name, config, handler)`. That signature has no `annotations` slot, so **nothing in this server declares which tools are reads and which are writes**: no `readOnlyHint`, no `destructiveHint`, no `idempotentHint`. A host cannot tell `skylight_list_events` from `skylight_delete_event` by inspection. This is known fleet debt, tracked here rather than fixed piecemeal — migrating 113 registrations is its own change with its own review, not a rider on an unrelated one. Any new tool should still use the old signature until that migration lands, so the file stays uniform and the migration stays one mechanical pass.
- **No `view: compact | full` parameter, and it is blocked on the line above.** The fleet's `view` rung strips image/avatar URLs from an upstream payload. Every read here *is* upstream-verbatim (`textContent(flattenJsonApi(await c.request(...)))`) so the premise holds — but two things stop it. First, there is nowhere to put the read/write distinction the rung depends on: with no annotations, "add `view` to the read tools" has no machine-checkable definition of *read tool*. Second, the rule would not pay even if wired: `stripMediaUrls`' key regex is built for camelCase (`photoUrl`, `thumbnailLink`) and Skylight's API is snake_case, so `profile_picture_urls` — the one media field this repo has actually verified (see `tools/members.ts`, "the server pushes it to Cloudinary and fills in `profile_picture_urls`") — is *kept*, not stripped. And the frame's photo tools (`skylight_list_messages`, `skylight_get_message`, `skylight_list_albums`, `skylight_list_avatars`) are tools whose PRODUCT is the image; stripping there empties the response instead of shrinking it. A `src/view.ts` was added by a fleet rollout, wired to nothing, and removed. Revisit after the `registerTool` migration, and with a field projection built from a captured payload — this repo has no fixtures, which is the thing to fix first.

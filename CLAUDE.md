...
- `src/lib.ts` — library entry point (`exports["."]` → `dist/lib.js`). Re-exports the thirteen tool registrars plus the client/auth seams so the tools can be hosted on another transport. Must never import `./index.js`, whose top-level `await runMcp(...)` would start a stdio server on import; `tests/library-exports.test.ts` guards that and the registrar-set drift.

...
- **Every API path that interpolates a value is built with the `apiPath` tagged template** (`src/tools/_shared.ts`), never a plain template literal. Ids come from the model, and `fetch` honours `?`, `#` and `../` in the path — an unencoded `id: '123?apply_to=all'` slipped a query past the `skylight_delete_chore` gate (fleet-audit#247). `apiPath` percent-encodes each value into one segment and refuses `.`/`..`/empty; `tests/tools/path-injection.test.ts` fails on any `/…${…}` literal that bypasses it. Query params go through `opts.query`.
- Tool return shape: `minifiedResult(...)` from `@chrischall/mcp-utils` — `{ content: [{ type: 'text', text: JSON.stringify(...) }] }`, one line, no indentation. The pretty-printed two-space form this line used to document is gone: indentation is tokens the caller pays for and never reads. Whitespace *inside* a value (a chore's note, a list item's text) is untouched — only the JSON framing is minified.
- **Tools register with `server.registerTool(name, config, handler)` and every one declares `annotations.readOnlyHint`** — all 113 of them, zero `server.tool(...)`. The migration off the old four-argument `server.tool(name, description, schema, handler)` signature landed as one mechanical pass; before it, the signature had no `annotations` slot, so nothing in this server declared which tools were reads and which were writes, and a host could not tell `skylight_list_events` from `skylight_delete_event` by inspection. The split is **36 reads / 77 writes**: `readOnlyHint: true` means the tool cannot change state on Skylight (it issues GETs only), and anything that creates, updates, deletes, invites, uploads, approves, redeems, marks-complete or sends is `readOnlyHint: false`. Seven writes additionally carry the `destructiveHint: true` they already had. A new tool must declare `readOnlyHint` explicitly; when the answer is genuinely unclear, `false` is the safe direction — `true` on a mutating tool is a false promise to every host that reads annotations.
- **No `view: compact | full` parameter.** The fleet's `view` rung strips image/avatar URLs from an upstream payload. Every read here *is* upstream-verbatim (`textContent(flattenJsonApi(await c.request(...)))`) so the premise holds — and the blocker named on the line above is now gone: `annotations.readOnlyHint` gives "add `view` to the read tools" a machine-checkable definition of *read tool*. What still stops it is that the rule would not pay even if wired: `stripMediaUrls`' key regex is built for camelCase (`photoUrl`, `thumbnailLink`) and Skylight's API is snake_case, so `profile_picture_urls` — the one media field this repo has actually verified (see `tools/members.ts`, "the server pushes it to Cloudinary and fills in `profile_picture_urls`") — is *kept*, not stripped. And the frame's photo tools (`skylight_list_messages`, `skylight_get_message`, `skylight_list_albums`, `skylight_list_avatars`) are tools whose PRODUCT is the image; stripping there empties the response instead of shrinking it. A `src/view.ts` was added by a fleet rollout, wired to nothing, and removed. Revisit with a field projection built from a captured payload — this repo has no fixtures, which is the thing to fix first.
...
Version appears in several places — all must match: `package.json`, `package-lock.json`, `src/index.ts` (the `createMcpServer({ version })` call, annotated with `// x-release-please-version`), `manifest.json`, `server.json`. The `tests/version-sync.test.ts` file asserts this. Don't bump manually unless explicitly asked — versioning is automated via release-please.
